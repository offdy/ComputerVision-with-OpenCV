...
def reorderPts(pts):
    ...
    # pts[:, 1] -> 배열의 모든 행에서 두번째 열 값을 추출 -> 배열의 모든 점에 대한 y좌표 값을 담은 벡터
    # pts[:, 0] -> 배열의 모든 행에서 첫번째 열 값을 추출 -> 배열의 모든 점에 대한 x좌표 값을 담은 벡터
    pts = pts[idx]  # x좌표로 정렬
...
# 영상 불러오기
filename = '/Users/ehddud/Desktop/Computer Vision/ch07/namecard1.jpg'
if len(sys.argv) > 1:
    filename = sys.argv[1]
